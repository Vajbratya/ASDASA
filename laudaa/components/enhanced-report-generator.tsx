...
import {
  ...
  ChevronLeft, ChevronRight, CornerUpLeft, CornerUpRight, Maximize,
  List, Code, Link, Image as ImageIcon, Table, Paperclip,
  Brain, Lightbulb, FileText, Wand2, Mic, Send, Search,
  Plus, Trash2, PenTool, Sliders, Settings, HelpCircle, LogOut,
  ChevronDown, Check, ChevronsUpDown, X, User, Loader2,
  LayoutTemplate, FileUp, AlertTriangle, BookOpen, Sparkles,
  Bot, PenBox, RefreshCcw, ThumbsUp, ThumbsDown, Clipboard, Share2,
  ArrowUpRight, ArrowDownRight, BarChart2, Calendar, FileQuestion,
  MessageSquare, Zap, BookMarked, Stethoscope, Activity
} from "lucide-react"
...
interface AppContextType {
  ...
  patientInfo: { name: string; id: string; dob: string; gender: string }
  setPatientInfo: React.Dispatch<React.SetStateAction<{ name: string; id: string; dob: string; gender: string }>>
  ...
  measurementHistory: { id: number; name: string; value: number; unit: string; date: Date }[]
  setMeasurementHistory: React.Dispatch<React.SetStateAction<{ id: number; name: string; value: number; unit: string; date: Date }[]>>
  addMeasurementHistory: (name: string, value: number, unit: string) => void
  removeMeasurementHistory: (id: number) => void
  updateMeasurementHistory: (id: number, value: number) => void
}
...
const Header = ({ toggleSidebar }: { toggleSidebar: () => void }) => {
  ...
  const { notifications, setNotifications, openUserProfile, recentReports } = appContext
  ...
  return (
    ...
    >
      <div className="flex items-center space-x-4">
        ...
        <span className="text-sm">NEXT-GEN RADIOLOGY REPORTING. </span>
      </div>
      <div className="flex items-center space-x-4">
...
const AutoTexto = () => {
  ...
  const checkConditions = useCallback((trigger: { id: number; trigger: string; content: string; tags: string[]; category: string; conditions: string[] }) => {
    return !trigger.conditions.some(condition => content.includes(condition))
  }, [content])
...
const Editor = () => {
  ...
  const { content, setContent, saveReport, exportReport, isDarkMode, autoTextoTriggers, templates, setTemplates, tags, setTags, patientInfo, setPatientInfo, studyInfo, setStudyInfo, clinicalHistory, setClinicalHistory, findings, setFindings, impressions, setImpressions, recommendations, setRecommendations, criticalFindings, setCriticalFindings, addFinding, removeFinding, addImpression, removeImpression, addRecommendation, removeRecommendation, addCriticalFinding, removeCriticalFinding, reportVersions, createReportVersion, switchReportVersion, compareReportVersions, referenceMaterials, viewReferenceMaterial, measurementTools, addMeasurement, removeMeasurement, updateMeasurement, voiceCommands, executeVoiceCommand, reportQualityScore, calculateReportQualityScore, customizationOptions, updateCustomizationOption, reportComments, addReportComment, removeReportComment, aiSuggestions, generateAiSuggestion, applyAiSuggestion, dismissAiSuggestion, measurementHistory, addMeasurementHistory, removeMeasurementHistory, updateMeasurementHistory } = appContext
  ...
  const [selectedUnit, setSelectedUnit] = useState<'mm' | 'cm'>('mm')
  const { toast } = useToast()
  ...
  const handleContentChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (trigger && !trigger.conditions.some(condition => newContent.includes(condition))) {
      const newContentWithAutoTexto = newContent.slice(0, -lastWord.length) + trigger.content
      setContent(newContentWithAutoTexto)
      e.target.setSelectionRange(newContentWithAutoTexto.length, newContentWithAutoTexto.length)
  ...
  const insertAutoTexto = useCallback((triggerContent: string) => {
    ...
    if (textarea) {
      ...
      const newContent = textBeforeCursor.substring(0, lastSpaceIndex + 1) + triggerContent + ' ' + textAfterCursor
      setContent(newContent)
      setShowAutoTexto(false)
      textarea.focus()
      const newCursorPosition = lastSpaceIndex + 1 + triggerContent.length + 1
  ...
  const handleMeasurementHistory = useCallback(() => {
    setShowMeasurementHistory(true)
  }, [])

  const convertMeasurement = useCallback((value: number, fromUnit: string, toUnit: string) => {
...
TECHNIQUE:
${studyInfo.modality} of the ${studyInfo.bodyPart}

FINDINGS:
${findings.join('\n')}

IMPRESSION:
Based on the clinical information and findings, the following impression can be made:
1. ${impressions[0] || 'No significant abnormalities detected.'}
${impressions.slice(1).map((imp, index) => `${index + 2}. ${imp}`).join('\n')}
...
${criticalFindings.length > 0 ? `CRITICAL FINDINGS:\n${criticalFindings.join('\n')}` : ''}
      ...
      setContent(aiGeneratedReport)
      toast({
        title: "AI Analysis Complete",
        description: "The AI has generated a report based on the structured data. Please review and edit as needed.",
      })
    } catch (error) {
      toast({
  ...
  }, [clinicalHistory, studyInfo, findings, impressions, recommendations, criticalFindings, setContent, toast])
  ...
  return (
    ...
    >
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-grow flex flex-col">
        <TabsList>
          ...
          <TabsTrigger value="structured">
            Structured Report
            <Badge variant="secondary" className="ml-2"> NOVIDADE</Badge>
          </TabsTrigger>
        </TabsList>
        <TabsContent value="write" className="flex-grow flex flex-col relative">
          <div className="flex space-x-2 mb-4">
            ...
            <Button variant="ghost" size="sm" aria-label="Measurement history" onClick={handleMeasurementHistory}>
              <BarChart2 className="h-4 w-4" />
            </Button>
          </div>
          <Textarea
            ref={textareaRef}
            className="flex-grow resize-none p-4 custom-highlight"
        ...
        <TabsContent value="preview" className="flex-grow">
          <div className={`prose ${isDarkMode ? 'dark:prose-invert' : ''} max-w-none h-full overflow-auto p-4`} style={{ width: '210mm', minHeight: '297mm', padding: '20mm', margin: '0 auto', boxShadow: '0 0 10px rgba(0,0,0,0.1)' }}>
            ...
            <div style={{ whiteSpace: 'pre-wrap', color: isDarkMode ? '#fff' : '#000' }}>{content}</div>
          </div>
        </TabsContent>
        <TabsContent value="structured" className="flex-grow">
          ...
          <div className="mt-6">
            <Button
              ...
              disabled={aiAnalysisLoading || !clinicalHistory || findings.length === 0}
              className="w-full"
            >
              {aiAnalysisLoading ? (
                <>
        ...
        <SheetContent>
          ...
          <div className="py-4">
            <div className="mb-4">
              ...
              <Select value={selectedUnit} onValueChange={(value: 'mm' | 'cm') => setSelectedUnit(value)}>
            ...
            {measurementTools.map((tool) => (
              <div key={tool.id} className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center">
                  <Input
                    type="number"
                    value={convertMeasurement(tool.value, tool.unit, selectedUnit)}
                    onChange={(e) => updateMeasurement(tool.id, convertMeasurement(parseFloat(e.target.value), selectedUnit, tool.unit))}
                    className="w-20 mr-2"
                  />
            ...
            <Button onClick={() => addMeasurement('New Measurement', 0, selectedUnit)} className="mt-4">
              Add Measurement
            </Button>
          </div>
        </SheetContent>
      ...
      <Sheet open={showReferenceMaterials} onOpenChange={setShowReferenceMaterials}>
        <SheetContent>
          <SheetHeader>
            <SheetTitle>Reference Materials</SheetTitle>
          </SheetHeader>
          <div className="py-4">
      ...
      <Sheet open={showMeasurementHistory} onOpenChange={setShowMeasurementHistory}>
        <SheetContent>
          <SheetHeader>
            <SheetTitle>Measurement History</SheetTitle>
          </SheetHeader>
          <div className="py-4">
            <div className="mb-4">
              <Label htmlFor="history-unit-select">Select Unit</Label>
              <Select value={selectedUnit} onValueChange={(value: 'mm' | 'cm') => setSelectedUnit(value)}>
                <SelectTrigger id="history-unit-select">
                  <SelectValue placeholder="Select unit" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="mm">Millimeters (mm)</SelectItem>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={measurementHistory.filter(m => m.date >= subYears(new Date(), 1)).map(m => ({
                ...m,
                value: convertMeasurement(m.value, m.unit, selectedUnit)
              }))}>
            ...
            {measurementHistory.filter(m => m.date >= subYears(new Date(), 1)).map((measurement) => (
              <Card key={measurement.id} className="mb-4">
                <CardHeader>
                  <CardTitle>{measurement.name}</CardTitle>
                </CardHeader>
                <CardContent>
                  <p>Value: {convertMeasurement(measurement.value, measurement.unit, selectedUnit).toFixed(2)} {selectedUnit}</p>
...
export default function EnhancedRadiologyReportGenerator() {
  ...
  const [measurementHistory, setMeasurementHistory] = useState<{ id: number; name: string; value: number; unit: string; date: Date }[]>([])
  const { toast } = useToast()
...
1. No acute cardiopulmonary process.`)
  ...
  const updateReportStatistics = useCallback(() => {
    setReportStatistics(prevStats => ({
      ...
      mostCommonModality: studyInfo.modality
    }))
  ...
  const addMeasurementHistory = useCallback((name: string, value: number, unit: string) => {
    const newMeasurement = {
      id: measurementHistory.length + 1,
      name,
      value,
      unit,
      date: new Date()
    }
    setMeasurementHistory(prevHistory => [...prevHistory, newMeasurement])
    toast({ title: "Measurement added to history", description: `New measurement "${name}" has been added to the history` })
  }, [measurementHistory.length, toast])

  const removeMeasurementHistory = useCallback((id: number) => {
    setMeasurementHistory(prevHistory => prevHistory.filter(m => m.id !== id))
    toast({ title: "Measurement removed from history", description: "The selected measurement has been removed from the history" })
  }, [toast])

  const updateMeasurementHistory = useCallback((id: number, value: number) => {
    setMeasurementHistory(prevHistory => prevHistory.map(m => m.id === id ? { ...m, value, date: new Date() } : m))
    toast({ title: "Measurement history updated", description: "The measurement in the history has been updated" })
  }, [toast])
        ...
        <Tooltip>
          ...
          <TooltipContent side="left" className="max-w-sm">
            ...
            <ul className="list-disc pl-4 space-y-2">
              ...
              <li><strong>Measurement History:</strong> Track and visualize measurement changes over time.</li>
            </ul>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
